...
export class CustomerController {
  ...
  async search(@Query('keyword') keyword: string, @Headers('x-request-id') requestId: string): Promise<unknown> {
    return this.forward(() => this.backend.get(`/internal/shipments?keyword=${encodeURIComponent(keyword ?? '')}`, requestId))
  }

  @Get('customers/:id')
  async detail(@Param('id') id: string, @Headers('x-request-id') requestId: string): Promise<unknown> {
    return this.forward(() => this.backend.get(`/internal/shipments/${encodeURIComponent(id)}`, requestId))
  }

  @Get('products')
  async products(@Query('customerId') customerId: string, @Headers('x-request-id') requestId: string): Promise<unknown> {
    return this.forward(() => this.backend.get(`/internal/pickup-slots?customerId=${encodeURIComponent(customerId ?? '')}`, requestId))
  }
